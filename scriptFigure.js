...
Promise.all(promises).then(function (values) {
    ...
    function courbe(incidentname, color) {
        ...
        /*if(incidentname == 'Autres'){
            for (var j = 0; j < values[0].length; i++) {
                if(values[0][j].Type != tabTitles[0] & values[0][j].Type != tabTitles[1] & values[0][j].Type != tabTitles[2].Type &
                    values[0][j].Type != tabTitles[3] & values[0][j].Type != tabTitles[4] & values[0][j].Type != tabTitles[5])
                {
                    if (values[0][j].Date == 2014)
                a++;
            else if (values[0][j].Date == 2015)
                b++;
            else if (values[0][j].Date == 2016)
                c++;
            else if (values[0][j].Date == 2017)
                d++;
            else
                e++;
                }
